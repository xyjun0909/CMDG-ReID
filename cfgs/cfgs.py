def parse_common_args(parser):
    ...
    parser.add_argument("--device_id", type=str, default="8")
    parser.add_argument("--num_workers", type=int, default=16)
    parser.add_argument("--num_instance", type=int, default=4)
    parser.add_argument(
        "--data_path", type=str, default="/home/nihao/data", help="dataset path"
    )
    parser.add_argument("--log_path", type=str, default="/home/nihao/data/exp", help="log path")
    parser.add_argument("--log_suffix", type=str, default="", help="Suffix for log directory")
    parser.add_argument(
        "--prior_img_epoch", type=int, default=3, help="prior epoch for image"
    )
    parser.add_argument(
        "--use-optimized", 
        action="store_true", 
        help="是否使用优化方案（预计算+高相似筛选），默认不使用"
    )
    parser.add_argument(
        "--top-k", 
        type=int, 
        default=100, 
        help="优化方案中高相似样本的数量，仅当--use-optimized=True时有效"
    )
    return parser
...
def parse_train_args(parser, datasets=None, classes=None, combine_all=False):
    ...
    parser.add_argument(
        "--prompt-domain-epoch", type=int, default=30, help="prompt domain epoch"
    )
    parser.add_argument(
        "--prior-text-epoch", type=int, default=7, help="prior epoch for text"
    )
    # parser.add_argument(
    #     "--prior_img_epoch", type=int, default=3, help="prior epoch for image"
    # )
    parser.add_argument(
        "--image-encoder-epoch", type=int, default=60, help="image encoder epoch"
    )
    parser.add_argument("--prompt-lr", default=0.00035, type=float)
    parser.add_argument("--image-encoder-lr", default=0.000005, type=float)
    parser.add_argument("--text-encoder-lr", default=0.000001, type=float)
    parser.add_argument("--beta", default=0.1, type=float, help="weight for apn-loss")
    parser.add_argument(
        "--omega", default=0.01, type=float, help="weight for domain loss"
    )
    parser.add_argument(
        "--epsilon",
        default=0.1,
        type=float,
        help="weights for gradient backpropagation",
    )
    parser.add_argument(
        "--lamda", default=0.01, type=float, help="learning rate for domain classifier"
    )
...
def protocol_1(parser, parsertest):
    return (
        parse_train_args(
            parser,
            ["Market", "cuhk_sysu", "CUHK02", "cuhk03"],
            [1502, 11934, 1816, 1467],
            True,
        ),
...
def protocol_2_C3(parser, parsertest):
    return (
        parse_train_args(
            parser, ["Market", "MSMT17", "cuhk_sysu"], [1502, 4101, 11934], False
        ),
...
def protocol_3_C3(parser, parsertest):
    return (
        parse_train_args(
            parser, ["Market", "MSMT17", "cuhk_sysu"], [1502, 4101, 11934], True
        ),
        parse_test_args(parsertest, ["cuhk03"]),
        "protocol_2_C3",
    )
...
def protocol_2_MS(parser, parsertest):
    return (
        parse_train_args(
            parser, ["cuhk03", "cuhk_sysu", "Market"], [1467, 11934, 1502], False
        ),
...
def protocol_3_MS(parser, parsertest):
    return (
        parse_train_args(
            parser, ["cuhk03", "cuhk_sysu", "Market"], [1467, 11934, 1502], True
        ),
        parse_test_args(parsertest, ["MSMT17"]),
        "protocol_3_MS",
    )
...
def protocol_2_M(parser, parsertest):
    return (
        parse_train_args(
            parser, ["cuhk03", "cuhk_sysu", "MSMT17"], [1467, 11934, 4101], False
        ),
...
def protocol_3_M(parser, parsertest):
    return (
        parse_train_args(
            parser, ["cuhk03", "cuhk_sysu", "MSMT17"], [1467, 11934, 4101], True
        ),
        parse_test_args(parsertest, ["Market"]),
        "protocol_3_M",
    )

def protocol_test_market(parser, parsertest):
    return (
        parse_train_args(
            parser, ["Market"], [1502], False
        ),
        parse_test_args(parsertest, ["Market"]),
        "protocol_test_market",
    )
