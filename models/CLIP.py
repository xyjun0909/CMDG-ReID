...
class TextEncoderForCaptions(nn.Module):
    def __init__(self, clip_model, fine_tune_layers=4):
        ...
        self.text_projection = clip_model.text_projection
        self.dtype = clip_model.dtype
        self.fine_tune_layers = fine_tune_layers  # 微调的顶层数量
        self.token_embedding = clip_model.token_embedding

        # 固定底层，微调顶层（保留CLIP基础语义，适配复杂文本）
        total_layers = len(self.transformer.resblocks)  # Transformer总层数

        for i, block in enumerate(self.transformer.resblocks):
            if i < (total_layers - self.fine_tune_layers):
                for param in block.parameters():
                    param.requires_grad = False  # 固定底层
            else:
                for param in block.parameters():
                    param.requires_grad = True  # 微调顶层
...
class Model(nn.Module):
    def __init__(self, num_classes, args, epsilon=.1, domain_num=4):
        ...
        # 归一化层
        self.img_norm = nn.LayerNorm(self.in_planes)  # 原始特征归一化（768维）
        self.img_norm_proj = nn.LayerNorm(self.in_planes_proj)  # 投影特征归一化（512维）
        ...
        self.text_encoder = TextEncoderForCaptions(clip_model)  # 提示词编码器（固定）
        # captions编码器（局部微调，微调最后4层Transformer）
        # self.captions_text_encoder = TextEncoderForCaptions(clip_model, fine_tune_layers=4)
        
        # self.local_proj = nn.Linear(self.in_planes, self.in_planes)  # 768→768投影
        # self.local_proj.apply(weights_init_kaiming)  # 初始化投影层
        # self.local_proj_proj = nn.Linear(self.in_planes_proj, self.in_planes_proj)  # 512→512投影
        # self.local_proj_proj.apply(weights_init_kaiming)

    def forward(self, x=None, label=None, get_image=False, get_text=False, cam_label=None, view_label=None,
                domain=None, prior=False, getdomain=False, captions=None, get_captions=False):
        if get_captions:
            return self.encode_captions(captions)  # [上, 中, 下, 全局]

        # if get_text:
        #     prompts = self.promptlearner(label)
        #     text_features = self.text_encoder(prompts, self.promptlearner.tokenized_prompts)
        #     dcscore = None
        #     return text_features, dcscore
        if get_text:
            if getdomain:
        ...
        if get_image:
            ...
            return region_features_proj  # [上, 中, 下, 全局]
    ...
    def get_image_features(self, x, image_features, image_features_proj):
        ...
        for i in range(self.num_regions):
            ...
            # region = self.local_proj(region)  # 投影后特征更具区分度
            # region_proj = self.local_proj_proj(region_proj)
        ...
        # 归一化
        region_features = [self.img_norm(f) for f in region_features]  # 768维特征用img_norm
        region_features_proj = [self.img_norm_proj(f) for f in region_features_proj]  # 512维特征用img_norm_proj
