...
from .samplers.sampler import DomainAwareIdentitySampler
from .trans import bulid_transforms


def collate_fn(batch):
    imgs, pids, camids, viewids, image_path, domains, cid,captions = zip(*batch)
    pids = torch.tensor(pids, dtype=torch.int64)
    viewids = torch.tensor(viewids, dtype=torch.int64)
    camids = torch.tensor(camids, dtype=torch.int64)
    cid = torch.tensor(cid, dtype=torch.int64)
    domains = torch.tensor(domains, dtype=torch.int64)
    return torch.stack(imgs, dim=0), pids, camids, viewids, domains, cid,captions,image_path
...
def build_data_loader(args, args_test):
    ...
    # sampling_method = DomainAwareIdentitySampler(train_items, args.batch_size, 4)
