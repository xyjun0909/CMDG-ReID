...
class Dataset(object):
    ...
    def __init__(self, train, query, gallery, transform=None, mode='train',
                 combineall=False, verbose=True,last_id=0, **kwargs):
        self.train = train
        self.query = query
    ...
    def combine_all(self):
        ...
        def _combine_data(data):
            for info in data:
                ...
                if info[1] in self._junk_pids:
                    continue
                #pdb.set_trace()
                # if info[3] not in info[1]:
                pid = self.dataset_name + "_" + str(info[1])
                # else:pid = info[1]
                # if info[3] not in info[2]:
                camid = self.dataset_name + "_" + str(info[2])
                # else:camid = info[2]
                combined.append((info[0], pid, camid,info[3],info[4]))
