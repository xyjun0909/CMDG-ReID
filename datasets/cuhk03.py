...
class cuhk03(ImageDataset):
    ...
    def __init__(self, root="", split_id=0, cuhk03_labeled=True, cuhk03_classic_split=False, **kwargs):
        ...
        if no_captions:
            ...
            required_files = [
                self.dataset_dir,  # cuhk03
                self.data_dir,   # cuhk03_release
                self.raw_mat_path, # cuhk-03.mat
                self.split_new_det_mat_path, # new detected .mat
                self.split_new_lab_mat_path # new labeled .mat
            ]
            ...
            gallery = self.process_dataset('gallery')
            query = self.process_dataset('query')
        ...
        if type == 'train':
          for item in json_data:
              if type in item.get('split', ''): 
                  ...
                  data.append((img_path, pid, camid, 'cuhk03', captions))
        else :
          if type == 'gallery':
              for item in json_data:
                  if type in item.get('split', ''): 
